...
public class SelectZhengceActivity extends Activity {
	...
	private TextView putongzhengce_tv, gaofanzhengce_tv,special_policy_status_tv;
	...
	private void blindListData(ArrayList<PolicyList> pList){
		...
		policy_lv.setItemActionListener(new ActionSlideExpandableListView.OnActionClickListener() {
			@Override
			public void onClick(View listView, View buttonview, int position) {
				String policy="";
				if (!isSpecialPolicy&&basePolicyLists.size()>position) {
					policy=JSONHelper.toJSON(basePolicyLists.get(position));
				}else if(isSpecialPolicy&&specialPolicyLists.size()>position){
					policy=JSONHelper.toJSON(specialPolicyLists.get(position));
				}
				setResult(SELECTED_POLICY_CODE, getIntent().putExtra(SELECTEDPOLICY, policy));
				finish();
			}
		}, R.id.select_btn);
	}
	...
	private class ListAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			time_section_tv.setText(str.get(position).getWtime().replace(",", "\r\n").replace(" ", ""));
