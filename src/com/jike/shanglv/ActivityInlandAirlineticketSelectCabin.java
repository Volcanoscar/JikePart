...
public class ActivityInlandAirlineticketSelectCabin extends Activity {
	...
	protected static final String TOKEN_NAME = "THE_FLIGHT_USER_SELECTED";//工作（用户选择）  航班
	...
	private TextView title_tv,startoff_date_tv,dayOfTheWeek_tv,PlaneTypeAndModel_tv,startoff_time_tv,arrive_time_tv,start_port_tv,arrive_port_tv,runtime_tv,oil_tax_tv;
	...
	private String startcity_code = "", arrivecity_code = "",
			...
			startoff_date = "", startdate = "", enddate = "";// 从搜索页面传递过来的数据
	...
	private void initView() {
		...
		PlaneTypeAndModel_tv.setText(ia.getPlaneType()+"("+ia.getPlaneModel()+")");
		...
		oil_tax_tv.setText("燃油+机建=￥"+(Integer.parseInt(ia.getOil())+Integer.parseInt(ia.getTax())));
	...
	private class ListAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			RelativeLayout fanMoney_rl=(RelativeLayout) convertView.findViewById(R.id.fanMoney_rl);
			ImageButton booking_imgbtn=(ImageButton) convertView.findViewById(R.id.booking_imgbtn);
			RelativeLayout tuiGaiQian_rl =(RelativeLayout) convertView.findViewById(R.id.tuiGaiQian_rl);
...
//			}
			...
			CabinName_tv.setText(str.get(position).getCabinName()+str.get(position).getCabin());
			discount_tv.setText(discountDeal(str.get(position).getDiscount()
					.trim()));
			fanMoney_tv.setText(" ￥"+str.get(position).getYouHui());
			price_tv.setText(" ￥" + str.get(position).getSale());
			ticketCount_tv.setText(tiecketCountDeal(str.get(position).getTicketCount()));
			...
			booking_imgbtn.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					  ...
					  if (wayType==SingleOrDouble.singleWay||wayType==SingleOrDouble.doubleWayGo) {
						  intentSend.putExtra(ActivityInlandAirlineticketBooking.ORDERWAYTYPE,
								  SingleOrDouble.singleWay);
						  intentSend.putExtra(ActivityInlandAirlineticketSelectCabin.TOKEN_NAME1,
								  jsonObject.toString());
						  intentSend.putExtra(ActivityInlandAirlineticketBooking.SELECTED_CABIN_INDEX1,
								  index);
...
//												intents.putExtra("wayType", SingleOrDouble.doubleWayBack);
...
//							  .show();
						  ...
						  intentSend.putExtra(ActivityInlandAirlineticketSelectCabin.TOKEN_NAME2,
								  jsonObject.toString());
						  intentSend.putExtra(ActivityInlandAirlineticketBooking.SELECTED_CABIN_INDEX2,
								  index);
						  intentSend.putExtra(ActivityInlandAirlineticketSelectCabin.TOKEN_NAME1,
								  goFlight);
						  intentSend.putExtra(ActivityInlandAirlineticketBooking.SELECTED_CABIN_INDEX1,
