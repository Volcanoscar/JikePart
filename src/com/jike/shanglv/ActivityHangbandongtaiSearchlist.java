...
public class ActivityHangbandongtaiSearchlist extends Activity implements
	...
	private void getIntentData(){
		...
		if (bundle!=null) {
			...
			if(bundle.containsKey("flightNo"))flightNo= bundle.getString("flightNo");
		}
	}
				...
				try {
					...
					if (state.equals("0000")) {
						...
						if(dongtai_List.size()==0)
							new AlertDialog.Builder(context).setTitle("未查相关的航班信息")
	...
	private void createList(JSONArray flist_list) {
		for (int i = 0; i < flist_list.length(); i++) {
			try {
				Hangbandongtai hb = JSONHelper.parseObject(flist_list.getJSONObject(i), Hangbandongtai.class);
				dongtai_List.add(hb);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private class ListAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			ImageView flight_company_logo=(ImageView) convertView.findViewById(R.id.flight_company_logo);
			...
			if (str.get(position).getRealfly()==null||str.get(position).getRealfly().trim().length()==0) {
				startTime_tv.setText(str.get(position).getPlanfly());
			}
			if (str.get(position).getRealreach()==null||str.get(position).getRealreach().trim().length()==0) {
