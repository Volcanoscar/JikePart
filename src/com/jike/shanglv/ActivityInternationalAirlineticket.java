...
public class ActivityInternationalAirlineticket extends Activity {
	...
	private TextView singleline_tv, doubleline_tv, startcity_tv, endcity_tv,
		startcity_code_tv, endcity_code_tv,startdate_tv, enddate_tv, startoff_date_tv;
	...
	View.OnClickListener btnClickListner = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.swith_city_iv:
				...
				startcity_tv.setText(endcity_tv.getText().toString().trim());
				endcity_tv.setText(tempCity);
				
				tempCityCode=startcity_code_tv.getText().toString().trim();
				startcity_code_tv.setText(endcity_code_tv.getText().toString().trim());
			...
			case R.id.chongzhi_button://搜索
				...
				Intent intents=new Intent(context,ActivityInternationalRequisitionForm.class);
				intents.putExtra("wayType", wayType);
				intents.putExtra("startcity", startcity_tv.getText().toString());
				intents.putExtra("arrivecity", endcity_tv.getText().toString());
				intents.putExtra("startcity_code", startcity_code_tv.getText().toString());
				intents.putExtra("arrivecity_code", endcity_code_tv.getText().toString());
				if(wayType == SingleOrDouble.singleWay)
					intents.putExtra("startoff_date", startoff_date_tv.getText().toString());
				else if(wayType == SingleOrDouble.doubleWayGo)
				{
					intents.putExtra("startdate", startdate_tv.getText().toString());
					intents.putExtra("enddate", enddate_tv.getText().toString());
				}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case startcity:
			if (b != null && b.containsKey("pickedCity")) {
				...
				startcity_code_tv.setText(myCity.substring(myCity.indexOf('#')+1));
			}
			break;
		case arrivecity:
			if (b != null && b.containsKey("pickedCity")) {
				...
				endcity_code_tv.setText(myCity.substring(myCity.indexOf('#')+1));
			}
			break;
		default:
			break;
	...
	protected void onSaveInstanceState(Bundle outState) {// 保存界面状态，记录点击TextView前是单程还是往返
		...
		outState.putSerializable("startcity_code_tv", startcity_code_tv.getText()
				.toString());
		outState.putSerializable("endcity_code_tv", endcity_code_tv.getText().toString());
	}
	...
	protected void onRestoreInstanceState(Bundle savedState) {// 根据之前保存的状态，恢复页面状态
		...
		startcity_tv.setText(savedState.getSerializable("startcity_tv")
				.toString());
		endcity_code_tv.setText(savedState.getSerializable("endcity_code_tv").toString());
		startcity_code_tv.setText(savedState.getSerializable("startcity_code_tv")
				.toString());
